"""
Create Figure 6: CDF of number of planets over accesible phase angle for
different inner working angles.
"""
...
def make_dual_cdf_plot(
    ...
    n_bins=20,
    savefig=False,
    filename="accessible_phase_angles.png",
):
    """
    ...
    for i, iwa in enumerate(iwa_list):
        ...
        count, bins_count = np.histogram(
            beta_maxes[:, :, i], bins=np.linspace(90, 180, 15)
        )
        ...
        axis.plot(
            bins_count[1:],
            (1 - cdf)
            * len(np.where(np.isfinite(beta_maxes[:, :, i]))[0])
            / beta_maxes[:, :, i].shape[0],
            color="C{:d}".format(i),
            linestyle="--",
            label=r"IWA = {:d} (mas)".format(iwa),
        )
        ...
        count, bins_count = np.histogram(
            betas[:, :, i].flatten() + 90, bins=np.linspace(90, 180, n_bins)
        )
        pdf = count / sum(count)
        cdf = np.cumsum(pdf)
        ...
        axis.plot(
            bins_count[1:],
            (1 - cdf) * np.sum(np.isfinite(beta_max2)),
            color="C{:d}".format(i),
            linestyle="-",
        )
    ...
    twinx.set_xticks(
        axis.get_xticks(),
        labels=[str(x) for x in 90 - (axis.get_xticks().astype(int) - 90)],
    )
    twinx.set_xlim(axis.set_xlim())
...
if __name__ == "__main__":
    ...
    for i in range(len(hab_zones)):
        ...
        incs = np.degrees(np.arccos(np.random.uniform(0.0, 1, size=n_sims)))
